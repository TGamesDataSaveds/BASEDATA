...
class Paths
{
	...
	private static var loadedAssets:Map<String, Bool> = new Map<String, Bool>();
	...
	public static function clearStoredMemory(?cleanUnused:Bool = false) {
		...
		localTrackedAssets = [];
		#if desktop openfl.Assets.cache.clear("songs"); #end
	}
	...
	{
		#if MODS_ALLOWED
		if(modsAllowed && !ClientPrefs.data.noneMods)
		{
	...
	{
		var sound:Sound = Sound.fromFile('./assets/sounds/${key}.${SOUND_EXT}');
		return sound;
	}

	static public function soundShared(key:String):Sound {
		var sound:Sound = Sound.fromFile('./assets/sounds/${key}.${SOUND_EXT}');
		return sound;
	}
	...
	{
		var file:Sound = Sound.fromFile('./assets/sounds/${key}.$SOUND_EXT');
		return file;
	}
	...
	{
		#if html5
		return 'songs:assets/preload/songs/${formatToSongPath(song)}/Inst.$SOUND_EXT';
		#else
	...
	static public function image(key:String, ?library:String = null, ?allowGPU:Bool = true):FlxGraphic
	{
		...
		var file:String = './assets/images/${key}.png';
		...
		if (FileSystem.exists(file)) {
			...
			trace('ARCHIVO [Assets/images/${key}.png] ENCONTRADO');
		} else {
			trace('ARCHIVO [Assets/images/${key}.png] NO ENCONTRADO');
		}
		}
	...
	static public function imageWeek(week:String, key:String, ?library:String = null, ?allowGPU:Bool = true):FlxGraphic
		{
			var bitmap:BitmapData = null;
			var newGraphic:FlxGraphic = null;
			var file:String = './assets/week_assets/${week}/${key}';
		...
		if (currentTrackedAssets.exists(file)) {
			...
			if (FileSystem.exists(file)) {
				bitmap = BitmapData.fromFile(file);
				trace('ARCHIVO [Assets/week_assets/${week}/${key}.png] ENCONTRADO');
			} else {
				trace('ARCHIVO [Assets/week_assets/${week}/${key}.png] NO ENCONTRADO');
			}
		}
	...
	static public function getTextFromFile(key:String, ?ignoreMods:Bool = false):String
	{
		#if sys
		#if MODS_ALLOWED
		if (!ignoreMods && !ClientPrefs.data.noneMods &&FileSystem.exists(modFolders(key)))
	...
	{
		#if MODS_ALLOWED
		if(!ignoreMods && !ClientPrefs.data.noneMods)
		{
	...
	inline static public function getSparrowAtlasWeek(key:String, week:String, ?library:String = null, allowGPU:Bool = true):FlxAtlasFrames
		{
			#if MODS_ALLOWED
			var imageLoaded:FlxGraphic = imageWeek(week, key, allowGPU);
			var xmlExists:Bool = false;
			...
			return FlxAtlasFrames.fromSparrow((imageLoaded != null ? imageLoaded : imageWeek(week, key, library, allowGPU)), (xmlExists ? File.getContent(xml) : getPath(week + 'images/$key.xml', library)));
			#else
			return FlxAtlasFrames.fromSparrow(imageWeek(key, library, allowGPU), getPath(week + 'images/$key.xml', library));
			#end
		}
