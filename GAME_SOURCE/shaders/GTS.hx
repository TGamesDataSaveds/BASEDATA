...
class GTS {
    ...
    public var shader:GlitchFlx = new GlitchFlx();
    public var multi(default, set):Float;


    function set_multi(value:Float):Float {
        multi = Math.max(0, Math.min(1, value));
		shader.colorMultiplier.value = [multi];
		return value;
    }
...
class GlitchFlx extends FlxShader {
    @:glFragmentHeader('vec3 mod289(vec3 x) {
      ...
      @:glfragmentSource('void mainImage( out vec4 fragColor, in vec2 fragCoord )
      {
