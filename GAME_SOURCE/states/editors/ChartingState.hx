...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Speed Aument', "Value 1: Speed Aument\nValue 2: Time to Charge\n\nEsto Cambiara la velocidad de la Cancion\ncon un flash y cambio animado\nUtiliza el Valor 1 Para poder Intercambiar entre las Velocidades\ny el Valor 2 para Cambiar la duracion de cambio de velocidad."],
		['Arrow Position Plus', "Value 1: x positioned aument\nValue 2: y positioned aument\nValue 2: y positioned aument\n\nCambia la posicion original de las fechas\nesto aumenta sus posicion original"],
		...
		['Cinematics', "Value 1: Mode\nValue 2: Speed\n\nValor 1: [true/false]\nValue 2: Speed type Float"],
		['fireMode', "Value 1: 1/0\nValue 2: couther\n\nvalue 1: \n\n1: ON\n0: OFF\n\nCouther type Float"],
		['outline', "\nAtivated Outline in Enemy Sprite\nOnly Used by ThonnyDev[ADMIN]"],
		['subState', "Tester"],
		['hitSong',  "!If SongHitAuto is Desactivated!\nLoad the function [AdditionalEffects]"]
	];
	...
	**/
	...
	override function create()
	{
		...
		else
		{
			...
			_song = {
				...
				mision5: false,
				BGFreeplay: '',
				beatSectionAuto: true,
				animCamera: FlxEase.linear,
				bfChange: false
			};
			addSection();
		...
		rightTxt = new FlxText(rightIcon.x, rightIcon.y + 5, 22, 'DAD', 12);
		rightTxt.scrollFactor.set(1, 1);
		add(rightTxt);

		leftTxt = new FlxText(leftIcon.x, leftIcon.y + 5, 22, 'BF', 12);
		...
		text =
		"F Para Establecer la camara en DAD
		\nG Para Establecer la camara en BF
	...
	var easeType:Array<String>;
	function addUtilUI():Void
		{
			easeType = ["linear", "elasticInOut", "circInOut"];

		var anim_camera = new FlxUIDropDownMenu(10, 30, FlxUIDropDownMenu.makeStrIdLabelArray(easeType, true), function(ease:String) {
			if (ease == 'linear') {
				_song.animCamera = FlxEase.linear;
			} else if (ease  == 'elasticInOut') {
				_song.animCamera = FlxEase.elasticInOut;
			} else if (ease == 'circInOut') {
				_song.animCamera = FlxEase.circInOut;
			} else {
				_song.animCamera = FlxEase.linear;
			}
		});
		anim_camera.selectedLabel = Std.string(_song.animCamera);
		blockPressWhileScrolling.push(anim_camera);

		var tab_group_util = new FlxUI(null, UI_box);
		tab_group_util.name = "Util";

		tab_group_util.add(anim_camera);
		tab_group_util.add(new FlxText(anim_camera.x, anim_camera.y - 15, 0, "Camera Ease:"));

		UI_box.addGroup(tab_group_util);
	...
	function addSongUI():Void
	{
		...
		*/
		...
		tab_group_song.add(new FlxText(player4DropDown.x, player4DropDown.y - 15, 0, 'BoyFriend 2:'));
	...
	var check_isBF:FlxUICheckBox;
	var check_isDAD:FlxUICheckBox;
	...
	function addSectionUI():Void
	{
		...
		check_isBF = new FlxUICheckBox(check_altAnim.x + 130, check_altAnim.y, null, null, "CAM BF", 100);
		check_isBF.name = 'check_isBF';
		check_isBF.checked = _song.notes[curSec].isbf;

		check_isDAD = new FlxUICheckBox(check_isBF.x, check_isBF.y + 22, null, null, "CAM DAD", 100);
		check_isDAD.name = 'check_isDAD';
		check_isDAD.checked = _song.notes[curSec].isdad;
		...
		tab_group_section.add(check_isBF);
		tab_group_section.add(check_isDAD);
	...
	function addDataUI()
	{
		...
		var levitationEnemy:FlxUICheckBox = new FlxUICheckBox(10, 190, null, null, "Levitation Enemy", 100);
		levitationEnemy.checked = (_song.levitationEnemy == true);
		levitationEnemy.callback = function()
		{
			_song.levitationEnemy = levitationEnemy.checked;
			updateGrid();
		};

		var beatSectionAuto:FlxUICheckBox = new FlxUICheckBox(10, 210, null, null, "Auto Beat Section", 100);
		beatSectionAuto.checked = (_song.beatSectionAuto == true);
		beatSectionAuto.callback = function()
		{
			_song.beatSectionAuto = beatSectionAuto.checked;
			updateGrid();
		};
		...
		tab_group_data.add(levitationEnemy);
		tab_group_data.add(beatSectionAuto);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'CAM BF':
					_song.notes[curSec].isbf = check.checked;
				case 'CAM DAD':
					_song.notes[curSec].isdad = check.checked;
			}
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.F) {
			if (!check_isDAD.checked) {
			check_isDAD.checked = _song.notes[curSec].isdad = true;
			} else {
			check_isDAD.checked = _song.notes[curSec].isdad = false;	
			}
		} else if (FlxG.keys.justPressed.G) {
			if (!check_isBF.checked) {
			check_isBF.checked = _song.notes[curSec].isbf = true;
			} else {
			check_isBF.checked = _song.notes[curSec].isbf = false;
			}
		}
		...
		if (!blockInput)
		{
			...
			if (check_isBF.checked == true && check_isDAD.checked == false) {
				sectionCAM = 'BF';
			}
			if (check_isBF.checked == false && check_isDAD.checked == true) {
				sectionCAM = 'DAD';
			}
			if (check_isBF.checked == true && check_isDAD.checked == true) {
				sectionCAM = 'BF/DAD';
			}
			if (check_isBF.checked == false && check_isDAD.checked == false) {
				sectionCAM = 'NONE';
			}
	...
	*/
	...
	function updateSectionUI():Void
	{
		...
		check_isBF.checked = sec.isbf;
		check_isDAD.checked = sec.isdad;
