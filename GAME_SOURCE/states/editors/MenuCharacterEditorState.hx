...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		txtOffsets.setFormat(Paths.font("new/BUND.otf"), 32, FlxColor.WHITE, CENTER);
		...
		tipText.setFormat(Paths.font("new/BUND.otf"), 16, FlxColor.WHITE, CENTER);
		...
		if(ClientPrefs.data.music == 'Disabled') {
			pauseMusic.loadEmbedded(Paths.music('none'), true, true);
		} else if (ClientPrefs.data.music != 'Disabled') {
	...
	function reloadSelectedCharacter() {
		if (Paths.getSparrowAtlas('menucharacters/' + characterFile.image) != null) {
