...
class Prompt extends MusicBeatSubstate
{
	...
	public function new(promptText:String='', defaultSelected:Int = 0, okCallback:Void->Void,cancelCallback:Void->Void,acceptOnDefault:Bool=false,option1:String=null,option2:String=null) 
	{
		...
		close();} );
		buttonAccept.alpha = 1;
		buttonNo = new FlxButton(633.3,450,op2,function(){if(cancelc != null)cancelc();
		close();});
		buttonNo.alpha = 1;
		FlxTween.tween(buttonAccept, {alpha: 1}, 0.3, {ease: FlxEase.circInOut});
		FlxTween.tween(buttonNo, {alpha: 1}, 0.3, {ease: FlxEase.circInOut});
		super();	
	}
	...
	override public function create():Void 
	{
		...
		FlxTween.tween(panelbg, {alpha: 1, "scale.x": 1, "scale.y": 1}, 0.5, {ease: FlxEase.circInOut});
		FlxTween.tween(panel, {alpha: 1, "scale.x": 1, "scale.y": 1}, 0.5, {ease: FlxEase.circInOut, onComplete: function(twn:FlxTween) {

			FlxTween.tween(textshit, {alpha: 1}, 0.2, {ease: FlxEase.circInOut});
		}});
	}
	/*
